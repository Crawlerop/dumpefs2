...
if __name__ == "__main__":
    ...
    ap.add_argument("-es", "--ecc-spare-offset", default=0, type=intorhex, help="Offset to spare (RIFF) or Page size (standard) when using ECC")
    ...
    ap.add_argument("-ea", "--ecc-algo", choices=["rs", "hamming20"], default="rs", help="Error correction algorithm (rs = Reed-Solomon, hamming20 = Qualcomm 20-bit hamming code)")

    ap.add_argument("-s", "--start-offset", type=intorhex, default=-1, help="Pointer to EFS2 filesystem (default: autodetect)")
    ...
    if args.cefs:
        s = CEFS(open(args.in_filename, "rb"), 0 if args.start_offset == -1 else args.start_offset, args.encoding)
